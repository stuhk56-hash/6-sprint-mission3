import { PrismaClient, ArticleComment } from '@prisma/client';
import { DEFAULT_LIMIT } from '../constants/index.js'; // 상수 임포트
import { ApiError } from '../constants/index.js';
...
export const getProductComments = async (
  req: Request<{ id: string }, any, any, { cursor: string; limit: string }>,
  res: Response
) => {
...
export const createArticleComment = async (req, res) => {
  const { id: articleId } = JSON.parse(req.params.id);
...
export const updateArticleComment = async (
  req: Request<{id: string}, ArticleComment, { content: string }>,
  res: Response<ArticleComment | { error: string }>
) => {
  const { id } = req.params;
  const { content } = req.body;
