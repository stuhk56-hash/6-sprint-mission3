...
/*
  ...
  - 현재 로그인한 유저 id가 1이라고 가정
*/
SELECT COUNT(DISTINCT l.product_id) AS total_count
FROM likes l
WHERE l.user_id = 1;
...
/*
  ...
  - 현재 로그인한 유저 id가 1이라고 가정
*/
INSERT INTO products (name, description, price, tag, image_path, user_id)
VALUES ('test', 'description', 10000, 'CLOTHES', 'image_path', '/images/sample-product.jpg', 1);
...
*/
SELECT p.*, COUNT(l.product_id) AS like_count
FROM products p
...
/*
  ...
  - 1번 상품 조회
*/
SELECT p.*, (SELECT COUNT(*) FROM likes WHERE product_id = p.product_id) AS like_count
FROM products p
...
SET name = 'test', 
    ...
    price = 10000, 
    tag = 'CLOTHES', 
    image_path = '/images/updated-product.jpg',
    updated_at = NOW()
WHERE id = 1;
